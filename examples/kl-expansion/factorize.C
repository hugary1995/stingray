...
{
public:
  CovarianceKernel() = default;
  virtual Real covariance(Real tau) const = 0;
};

class SquaredExponential : public CovarianceKernel
{
public:
  SquaredExponential(Real L) : CovarianceKernel(), _L(L) {}
  virtual Real covariance(Real tau) const override
  {
    return std::exp(-M_PI / 4 * tau * tau / _L / _L);
  }
...
// the eigen system.
void assemble(EquationSystems & es, const CovarianceKernel & cov_kernel);
...
main(int argc, char ** argv)
{
  ...
  mesh.read(argv[1]);
  mesh.print_info();
  SquaredExponential cov_kernel(std::stod(argv[2]));

  // Create an equation systems object.
  EquationSystems equation_systems(mesh);

  // Create an EigenSystem named "Eigensystem"
  ...
  auto kl_sparsity = std::make_shared<KLSparsity>(mesh);
  eigen_system.get_dof_map().add_coupling_functor(kl_sparsity);

  // Set necessary parameters used in EigenSystem::solve(), i.e. the number of requested eigenpairs
  // nev and the number of basis vectors ncv used in the solution algorithm. Note that ncv >= nev
  // must hold and ncv >= 2*nev is recommended. Here we guess 1/10 eigenvalues will give us a good
  // spectrum.
  ...
  // Set the type of the problem, here we deal with a generalized Hermitian problem.
  ...
  // Assemly
  assemble(equation_systems, cov_kernel);
  ...
  for (unsigned int eigval_num = 0; eigval_num < nconv; eigval_num++)
  {
    std::pair<Real, Real> eigval = eigen_system.get_eigenpair(eigval_num);
    eigval_sum += eigval.first;
  }
  libMesh::out << "sum of eigenvalues: " << eigval_sum << std::endl;
  ...
  for (unsigned int eigval_num = 0; eigval_num < nconv; eigval_num++)
  {
    ...
    eigval_cum += eigval.first;
    if ((eigval_sum - eigval_cum) / eigval_sum < TOL)
    {
...
void
assemble(EquationSystems & es, const CovarianceKernel & cov_kernel)
{
  ...
  // Get a reference to our system. Get a constant reference to the Finite Element type for the
  // first (and only) variable in the system.
  auto & eigen_system = es.get_system<EigenSystem>("Eigensystem");
  auto fe_type = eigen_system.get_dof_map().variable_type(0);

  // A reference to the two system matrices
  auto & matrix_A = *eigen_system.matrix_A;
  auto & matrix_B = *eigen_system.matrix_B;

  // Build a Finite Element object of the specified type.  Since the FEBase::build() member
  // dynamically creates memory we will store the object as a std::unique_ptr<FEBase>.  This can be
  // thought of as a pointer that will clean up after itself.
  ...
  const auto & JxW_remote = fe_remote->get_JxW();

  // The element shape functions evaluated at the quadrature points.
  const auto & phi = fe->get_phi();
  const auto & phi_remote = fe_remote->get_phi();

  // The element shape function gradients evaluated at the quadrature points.
  const auto & dphi = fe->get_dphi();

  // A reference to the DofMap object for this system.  The DofMap object handles the index
  // translation from node and element numbers to degree of freedom numbers.
  const auto & dof_map = eigen_system.get_dof_map();
  ...
  const auto & q_points_remote = fe_remote->get_xyz();

  // Now we will loop over all the elements in the mesh that live on the local processor. We will
  // compute the element matrix and right-hand-side contribution.  In case users later modify this
  // program to include refinement, we will be safe and will only consider the active elements;
  // hence we use a variant of the active_elem_iterator.
  const auto total = mesh.n_active_local_elem();
  auto count = 0;
  ...
  for (const auto & elem : mesh.active_local_element_ptr_range())
  {
    ...
    Me.resize(n_dofs, dof_indices.size());

    // Now loop over the quadrature points.  This handles the numeric integration.
    // We will build the element matrix.  This involves a nested loop to integrate the test
    // functions (i) against the trial functions (j).
    ...
    for (unsigned int qp = 0; qp < qrule.n_points(); qp++)
      for (unsigned int i = 0; i < n_dofs; i++)
        for (const auto & elem_remote : mesh.active_element_ptr_range())
        {
          ...
          const auto n_dofs_remote = dof_indices_remote.size();
          Ke.resize(n_dofs, n_dofs_remote);
          for (unsigned int qp_remote = 0; qp_remote < qrule.n_points(); qp_remote++)
          {
            auto lag = q_points[qp] - q_points_remote[qp_remote];
            Real R = cov_kernel.covariance(lag.norm());
